...
func TestPollAPIServer_RecreateMode(t *testing.T) {
	...
	client, err := k8sclient.NewK8sClient("default", "deployment/test-dep", kubeconfig, false, k8sclient.ResizeModeRecreate, k8sclient.ResizeFallbackConfig{}, 10*time.Second)
	if err != nil {
...
func TestPollAPIServer_InPlaceMode(t *testing.T) {
	...
	kubeconfig := writeTempKubeconfig(t, server.URL)
	defer os.Remove(kubeconfig)
	client, err := k8sclient.NewK8sClient("default", "deployment/test-dep", kubeconfig, false, k8sclient.ResizeModeInPlace, k8sclient.ResizeFallbackConfig{}, 10*time.Second)
	if err != nil {
		t.Fatalf("NewK8sClient: %v", err)
	}
